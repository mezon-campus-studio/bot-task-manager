...
export class CreateTableV21777438687949 implements MigrationInterface {
  name = 'CreateTableV21777438687949';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD "role" "public"."users_role_enum" DEFAULT '3'`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."users_status_enum" AS ENUM('active', 'inactive', 'deleted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "status" "public"."users_status_enum" DEFAULT 'active'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "teams" ADD "status" "public"."teams_status_enum" NOT NULL DEFAULT 'active'`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_users_current_project_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN "current_project_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "current_project_id" character varying`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD "current_project_id" integer`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_users_current_project_id" ON "users" ("current_project_id") `,
    );
